...
function filterDemoApps({limit=100,featured,trending,type,categorySlug,q,status='published'}={}){
  let rows=[...demoApps];
  if(status)rows=rows.filter(a=>a.status===status);
  if(featured!==undefined)rows=rows.filter(a=>a.featured===featured);
  if(trending!==undefined)rows=rows.filter(a=>a.trending===trending);
  if(type)rows=rows.filter(a=>a.app_type===type);
  if(categorySlug)rows=rows.filter(a=>a.category?.slug===categorySlug);
  if(q){const x=q.toLowerCase();rows=rows.filter(a=>`${a.title} ${a.short_description} ${a.developer}`.toLowerCase().includes(x));}
  return rows.slice(0,limit);
}
async function hasPublishedDbApps(){
  if(!configured)return false;
  const [rows]=await db().query("SELECT COUNT(*) AS total FROM apps WHERE status='published'");
  return Number(rows?.[0]?.total||0)>0;
}
export async function getApps(opts={}){
  const {limit=100,featured,trending,type,categorySlug,q,status='published'}=opts;
  if(!configured) return filterDemoApps(opts);
  ...
  if(!rows.length && !(await hasPublishedDbApps())) return filterDemoApps(opts);
  return rows.map(r=>{const a=decodeRow(r); a.category=r.c_id?{id:r.c_id,name:r.c_name,slug:r.c_slug,description:r.c_description}:null; delete a.c_id;delete a.c_name;delete a.c_slug;delete a.c_description; return a;});
}
export async function getAppBySlug(slug){
  if(!configured)return demoApps.find(a=>a.slug===slug)||null;
  const [rows]=await db().execute('SELECT a.*,c.id c_id,c.name c_name,c.slug c_slug,c.description c_description FROM apps a LEFT JOIN categories c ON c.id=a.category_id WHERE a.slug=? LIMIT 1',[slug]);
  if(rows[0]){const r=rows[0];const a=decodeRow(r);a.category=r.c_id?{id:r.c_id,name:r.c_name,slug:r.c_slug,description:r.c_description}:null;return a;}
  if(!(await hasPublishedDbApps())) return demoApps.find(a=>a.slug===slug)||null;
  return null;
}
